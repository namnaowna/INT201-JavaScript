...
class Movies {
  ...
  addMovie(title, director, year, genre) {
    ...
    if (
      ...
      this.movies.find(
        (el) => el.director.toLowerCase() === director.toLowerCase()
      ) === undefined
    ) {
      return undefined
    }
  ...
  updateMovie(title, updatedDetails) {
    let findMovie = this.movies.findIndex(
      (el) => el.title.toLowerCase() === title.toLowerCase()
    )
    if (findMovie === -1) {
      return undefined
    ...
    if (
      Object.keys(updatedDetails).find((el) => el === "title") !== undefined
    ) {
      this.movies[findMovie].title = updatedDetails.title
    }
    if (
      Object.keys(updatedDetails).find((el) => el === "director") !== undefined
    ) {
    ...
    if (
      Object.keys(updatedDetails).find((el) => el === "genre") !== undefined
    ) {
