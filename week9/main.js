const add = (num1, num2) => num1 + num2;
const subtract = (num1, num2) => num1 - num2;

function doSomething1(num1, num2, op) {
  return op(num1, num2); //return result function
}
function doSomething2(op) {
  return op; //return function
}

const result1 = doSomething1(10, 5, add);
console.log("result1", result1); //result1 15

const result2 = doSomething2(add); //add function
console.log(typeof result2); //function
console.log("result2", result2); //result2 [Function: add]

const result3 = doSomething2(subtract);
console.log(typeof result3); //function
console.log("result3", result3); //result3 [Function: subtract]

const result4 = doSomething2(1);
console.log(typeof result4); //number
console.log("result4", result4); //result4 1

const result5 = doSomething2("add");
console.log(typeof result5); //string
console.log("result5", result5); //result5 add | add from String
